      ...
      >
        <to-form-item label="归属单位" prop="enterpriseCode">
          <SelectSearch
            ...
            label="name"
            :area-code="unitAreaCode"
            value="enterpriseCode"
            placeholder="请选择归属单位"
          />
          <!-- <to-select v-model="ruleForm.enterpriseCode" filterable placeholder="请选择归属单位">
            <to-option
...
import { DeptAdd, verytyDept } from '../../api/setting'
...
export default {
  ...
  mounted() {
    if (this.$route.query.areaCode) {
      this.unitAreaCode = this.$route.query.areaCode
      this.ruleForm.enterpriseCode = this.$route.query.enterpriseCode
    } else {
  ...
  methods: {
    ...
    async DeptSubmit() {
      const res = await DeptAdd(this.ruleForm)
      if (res.code == 0) {
        this.$message({ type: 'success', message: '部门新增成功' })
        this.$router.go(-1)
      }
    },
    // 提交
